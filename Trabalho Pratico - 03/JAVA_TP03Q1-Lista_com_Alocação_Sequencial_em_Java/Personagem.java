    ...
    public static void main(String args[]){
        ...
        try{
            ...
            for(int i = 0; i < number; i++) {
                entrada = br.readLine();
                String[] metodos = entrada.split(" ");

                if ("II".equals(metodos[0])) {
                    lista.inserirInicio(personagens.get(UUID.fromString(metodos[1])));
                } else if ("IF".equals(metodos[0])) {
                    lista.inserirFim(personagens.get(UUID.fromString(metodos[1])));
                } else if ("R*".equals(metodos[0])) {
                    System.out.println("(R) " + lista.remover(Integer.parseInt(metodos[1])).getName());
                } else if ("RI".equals(metodos[0])) {
                    System.out.println("(R) " + lista.removerInicio().getName());
                } else if ("RF".equals(metodos[0])) {
                    System.out.println("(R) " + lista.removerFim().getName());
                } else if ("I*".equals(metodos[0])) {
                    lista.inserir(personagens.get(UUID.fromString(metodos[2])), Integer.parseInt(metodos[1]));
                }
            }
            lista.mostrar();
        }catch(Exception e){
...
class Lista {
    ...
    void mostrar(){
        for(int i = 0; i < n; i++){
            ...
            String alternateNamesStr = "{";
            if (array[i].getAlternate_names() != null && !array[i].getAlternate_names().isEmpty()) {
                for(int j = 0; j < array[i].getAlternate_names().size(); j++){
                    alternateNamesStr += array[i].getAlternate_names().get(j) + ",";
                }
                alternateNamesStr = alternateNamesStr.replace("],", "}");
                alternateNamesStr = alternateNamesStr.replace("[", "");
            }else{
            ...
            DateTimeFormatter desiredFormatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
            String formattedDate = array[i].getDateOfBirth().format(desiredFormatter);

            System.out.println("[" + i + " ## " + array[i].getId() + " ## " + array[i].getName() + " ## " + alternateNamesStr + " ## " + array[i].getHouse()
                    + " ## " + array[i].getAncestry() + " ## " + array[i].getSpecies() + " ## " + array[i].getPatronus()
                    + " ## " + array[i].isHogwartsStaff() + " ## " + array[i].getHogwartsStudent() + " ## "
                    + array[i].getActorName() + " ## " + array[i].isAlive() + " ## " + formattedDate + " ## "
                    + array[i].getYearOfBirth() + " ## " + array[i].getEyeColour() + " ## " + array[i].getGender() + " ## "
                    + array[i].getHairColour() + " ## " + array[i].isWizard() + "]");
        }
    }
}