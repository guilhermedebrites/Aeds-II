...
public class Personagem{
    ...
    public static void main(String args[]){
        ...
        try{
            ...
            arrayPersonagens = ordenaSelecao(arrayPersonagens);
            for(int i = 0; i < 10; i++){
    ...
    public static ArrayList<Personagem> ordenaSelecao(ArrayList<Personagem> personagens){
        for(int i = 0; i < 10; i++){
            Personagem menor = personagens.get(i);
            int indiceMenor = i;

            for(int j = (i + 1); j < personagens.size(); j++){
                if(personagens.get(j).getName().compareTo(menor.getName()) < 0){
                    menor = personagens.get(j);
                    indiceMenor = j;
                }
            }

            Personagem aux = personagens.get(i);
            personagens.set(i, menor);
            personagens.set(indiceMenor, aux);
        }
        return personagens;
    }
