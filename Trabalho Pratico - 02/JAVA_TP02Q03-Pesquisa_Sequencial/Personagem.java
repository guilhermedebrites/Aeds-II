...
public class Personagem{
    ...
    public static void main(String args[]){
        ...
        try{
            entrada = br.readLine();
            while(!entrada.equals("FIM")){
                System.out.println(pesquisaSequencial(arrayPersonagens, entrada) ? "SIM" : "NAO");
                entrada = br.readLine();
            }
        }catch(Exception e){
            e.printStackTrace();
    ...
    public static boolean pesquisaSequencial(ArrayList<Personagem> personagens, String name) throws IOException{
        FileWriter escritor = new FileWriter("808721_sequencial.txt");
        ...
        for(int i = 0; i < personagens.size(); i++){
            contadorComparacoes++;
            if(name.equals(personagens.get(i).getName())){
                resp = true;
                i = personagens.size();
            }
        }
