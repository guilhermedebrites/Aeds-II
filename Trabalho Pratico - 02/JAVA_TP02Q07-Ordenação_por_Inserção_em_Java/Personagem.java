...
public class Personagem{
    ...
    public static void main(String args[]){
        ...
        try{
            ...
            arrayPersonagens = ordenaInsercao(arrayPersonagens);
    ...
    public static ArrayList<Personagem> ordenaInsercao(ArrayList<Personagem> personagens){
        
        for(int i = 1; i < personagens.size(); i++){
            Personagem temp = personagens.get(i);
            int j = i - 1;

            while((j >= 0) && (personagens.get(j).getDateOfBirth().isAfter(temp.getDateOfBirth()) || 
            (personagens.get(j).getDateOfBirth().isEqual(temp.getDateOfBirth()) && personagens.get(j).getName().compareTo(temp.getName()) > 0))){
                personagens.set(j + 1, personagens.get(j));
                j--;
            }
            personagens.set(j + 1, temp);
        }
