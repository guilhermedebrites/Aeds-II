...
public class Personagem{
    ...
    public static void main(String args[]){
        ...
        try{
            ...
            ordenaCounting(arrayPersonagens);
    ...
    public static void ordenaCounting(ArrayList<Personagem> personagens) {
        int[] count = new int[getMaior(personagens) + 1];
        Personagem[] ordenado = new Personagem[personagens.size()];
        ...
        for (int i = personagens.size() - 1; i >= 0; i--) {
            ...
            int index = count[year] - 1;
            ordenado[index] = p;
            count[year]--;
        }
    
        for (int i = 0; i < ordenado.length; i++) {
            personagens.set(i, ordenado[i]);
        }
    }
    
    public static int getMaior(ArrayList<Personagem> personagens) {
        int maior = personagens.get(0).getYearOfBirth();
        for (Personagem p : personagens) {
            if (p.getYearOfBirth() > maior) {
